...
- On spacious screens, the homepage directory becomes a bounded scroll-driven depth carousel: five destination cards travel left through a vertical depth path while staying front-facing. When scrolling pauses, the existing rotation eases the nearest destination into the highlighted center position before the section releases naturally into the footer. The document remains the only scroll container.
